...
- 语句（statement）：类似于平时说话时的一句话，由表达式组成，以`;`结束。`1+2;` `a+b;` `System.out.println("Hello World!");` 都是语句。
...
### 浮点（小数）类型
- `float` 占用4个 byte，有精度，值域复杂 ±340282346638528859811704183484516925440
...
运算符的运算结果类型有的是固定的，有时候会根据被计算的值变化。比如两个 `int` 相加，结果的类型就是 `int`；两个 byte 相加，返回值的类型是 `int`。
...
- 算数运算符
  - 当"+"操作中出现字符串时，此时为字符串连接符，而不是算数运算符了，会将前后的数据进行拼接，并返回一个新的字符串
  - 当`字符+字符`或者`字符+数字`时，会把字符通过ASCII码表查询到对应的数字再进行运算
- 取模运算符(用来计算余数)
- 比较运算符(>、>=、<、<=、!=、==)
- 布尔运算符(!、&、&&、|、||)
- 小括号运算符(决定运算符的顺序)
...
boolean isStudent = (data & is_student_mask) != 0; // true (0101 & 0001)
boolean isProgrammer = (data & is_programmer_mask) != 0; // false (0101 & 0010)
boolean isDriver = (data & is_driver_mask) != 0; // true (0101 & 0100)
boolean isPainter = (data & is_painter_mask) != 0; // false (0101 & 1000)
```
...
- `byte` `short` `char`三种类型的数据再运算的时候，都会直接先提升为`int`，然后再进行远算
### 强制类型转换
...
short a3 = 1;
a3 += 1; // `a3 + 1` 返回的是 int 类型，`a3 += 1;` 操作等同于 `a3 = (short)(a3 + 1);`
...
- 数值计算一旦溢出，结果将失去其原有意义。比如两个正数会加出负数。
  ```java
  int a = 2000000000;
  System.out.println(a + a); // -294967296
  ```
- 要对能够处理的值有大概的估计。
- BigInt
